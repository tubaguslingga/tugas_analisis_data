class DataAnalyzer:
    ...
    def create_sum_spend_df(self):
        sum_spend_df = self.df.resample(rule='ME', on='order_approved_at').agg({
        "payment_value":"sum"
        })
        sum_spend_df.index = sum_spend_df.index.strftime('%B')
        ...
        sum_spend_df["month_numeric"] = sum_spend_df["order_approved_at"].map(month_mapping)
        sum_spend_df = sum_spend_df.sort_values("month_numeric")
        sum_spend_df = sum_spend_df.drop("month_numeric", axis=1)
    ...
    def customer_count_by_city(self):
        df_bycity = self.df['customer_city'].value_counts().head(10)
        most_common_city = df_bycity.idxmax()
        df_bycity = df_bycity.sort_values(ascending=False)
        
        return df_bycity, most_common_city
